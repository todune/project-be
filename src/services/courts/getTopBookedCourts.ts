  ...
  try {
    ...
    const topCourts = await Booking.findAll({
      attributes: [
        ...
        [Sequelize.col('courtBookingData.name'), 'courtName'],
        [Sequelize.col('courtBookingData.catCourtData.name'), 'categoryName'],
      ...
      group: [
        'court_id', 
        'courtBookingData.id',
        'courtBookingData.catCourtData.id'
      ],
    ...
    const formattedData = topCourts.map(court => ({
      id: court.dataValues.courtId,
      name: court.dataValues.courtName,
      category: court.dataValues.categoryName,
      bookings: parseInt(court.dataValues.bookingCount, 10)
    }))
